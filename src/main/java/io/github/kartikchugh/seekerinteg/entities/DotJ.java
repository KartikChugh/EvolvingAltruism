...
public class DotJ extends Entity {
     ...
     */
    public DotJ(double posX, double posY, GenomeJ genome) {
        super(posX, posY);
        posX_start = posX;
        posY_start = posY;
        this.genome = genome;
     ...
     * @param posY starting y position
     */
    DotJ(int genes, double posX, double posY) {
        this(posX, posY, new GenomeJ(genes));
    }
     ...
     * @return cloned dot
     */
    DotJ cloned(double mutationChance) {
        return new DotJ(this.posX_start, this.posY_start, genome.cloned(mutationChance));
    }
    ...
    public void update() {
        ...
        // JENETICS edit - removed || isTouchingWall() || touchingGoal
        if (!genome.hasNextAcc() ) {
            moving = false; 
        } else {
            move();
            steps++;
